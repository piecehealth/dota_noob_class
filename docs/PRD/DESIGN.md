...
同步来源有优先级之分，高优先级来源可覆盖低优先级已入库数据：**系统拉取 > 维护者上传 > 学员自同步**。

**方式一：后端定时同步（系统拉取，最高优先级）**
...
**方式三：维护者 API 批量上传（中优先级）**
维护者通过后端 API 端点批量上传 match 数据（如补录历史数据、修复缺失记录）。
需携带维护者身份凭证（assistant 角色），数据格式与 OpenDota API 响应一致。
